                ...
                if (variables.indexOf(Postfix.substring(count,count+1) )!=-1)
                ...
                else
                   {
                    switch (Postfix.charAt(count) )     
                       {
                        case '-': { result = numbers[stackTop-1] - numbers[stackTop]; break; }
                        case '+': { result = numbers[stackTop-1] + numbers[stackTop]; break; }
                        case '*': { result = numbers[stackTop-1] * numbers[stackTop]; break; }
                        case '/': { result = numbers[stackTop-1] / numbers[stackTop]; break; }
                        case '^': { result = Math.pow(numbers[stackTop-1],numbers[stackTop]); break; }
                        case '\\': { result = numbers[stackTop-1] % numbers[stackTop]; break; }

                        case 's': { result = Math.sin(mul*numbers[stackTop]); stackTop++; break; }
                        case 'c': { result = Math.cos(mul*numbers[stackTop]); stackTop++; break; }
                        case 't': { result = Math.tan(mul*numbers[stackTop]); stackTop++; break; }
                        case 'l': { result = Math.log(numbers[stackTop]); stackTop++; break; }
                        case 'a': { result = mulx*Math.asin(numbers[stackTop]); stackTop++; break; }
                        case 'b': { result = mulx*Math.acos(numbers[stackTop]); stackTop++; break; }
                        case 'd': { result = mulx*Math.atan(numbers[stackTop]); stackTop++; break; }
                        case 'e': { result = Math.exp(numbers[stackTop]); stackTop++; break; }
                       }
                    stackTop=stackTop-1;        
