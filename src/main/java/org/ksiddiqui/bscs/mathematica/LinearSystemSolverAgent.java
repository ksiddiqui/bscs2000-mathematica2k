...
public class LinearSystemSolverAgent
{
        ...
        LinearSystemSolverAgent()
          {        
           ...
           detField = new JTextField(); detField.setEditable(false);
           detButton.addActionListener(new ActionListener() {
                public void actionPerformed(ActionEvent e) { detCommand(); }
                });
           ...
           m22.add(tolLabel); m22.add(tolField); 
           m22.add(iterationLabel); m22.add(iterationField); 
           m22.add(showSeq);
        ...
        private void execute(int m) 
          {
           ...
           try {  for (int i=0; i<rows; i++) valuesTableModel[m-1].removeRow(0); }  catch(Exception e) {}
