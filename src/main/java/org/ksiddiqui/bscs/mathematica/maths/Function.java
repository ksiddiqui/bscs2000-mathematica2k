...
/**/            else if (operands.indexOf(str2)!=-1)
                   {
                    ...
                    while(true)
                      {
                       ...
                       if      (tmpChar=='E' && !isSciNotation)  
                               { isSciNotation=true; power=""; count++; tmpChar=infixCopy[count];}
                    ...
                    variable++;
                    if (negFlag) { numberX=numberX*(-1);   negFlag=false;  }    
...
/**/            else if (operators.indexOf(str2)!=-1)
                   {                    
                    ...
                    if (pre1<10) opStack.push(new Character(tmpChar));
                       ...
                       {
                        int inc=0;
                        char ch='a'; char tmpChar1=infixCopy[count+1];
                        ...
                        else if (tmpChar=='a' && tmpChar1=='s' ) { ch='a'; inc=3; }
                        else if (tmpChar=='a' && tmpChar1=='c' ) { ch='b'; inc=3; }
                        else if (tmpChar=='a' && tmpChar1=='t' ) { ch='d'; inc=3; }
                        else if (tmpChar=='a' && tmpChar1=='l' ) { ch='e'; inc=3; }
                    ...
                    switch (Postfix.charAt(count) )     
                       {
                        ...
                        case '*': { result = n2 * n1; break; }
                        case '/': { result = n2 / n1; break; }
                        case '^': { result = Math.pow(n2,n1); break; }
                        case '\\':{ result = n2 % n1; break; }

                        case 's': { result = Math.sin(mul*n1); break; }
                        case 'c': { result = Math.cos(mul*n1); break; }
                        case 't': { result = Math.tan(mul*n1); break; }
                        case 'l': { result = Math.log(n1); break; }
                        case 'a': { result = mulx*Math.asin(n1); break; }
                        case 'b': { result = mulx*Math.acos(n1); break; }
                        case 'd': { result = mulx*Math.atan(n1); break; }
                        case 'e': { result = Math.exp(n1); break; }
                       }
           ...
           for (int count=0; count<Postfix.length(); count++)
               {
                ...
                if (Postfix.charAt(count) >= 'A' && Postfix.charAt(count) <= 'Z')
                ...
                else
                   {
                    switch (Postfix.charAt(count) )     
                       {
                        case '-': { postfixStr=postfixStr.append('-'); break; }
                        case '+': { postfixStr=postfixStr.append('+'); break; }
                        case '*': { postfixStr=postfixStr.append('*'); break; }
                        case '/': { postfixStr=postfixStr.append('/'); break; }
                        case '^': { postfixStr=postfixStr.append('^'); break; }
                        ...
                        case 's': { postfixStr=postfixStr.append("Sin"); break; }
                        case 'c': { postfixStr=postfixStr.append("Cos"); break; }
                        case 't': { postfixStr=postfixStr.append("Tan"); break; }
                        case 'l': { postfixStr=postfixStr.append("Log"); break; }
                        case 'a': { postfixStr=postfixStr.append("aSin"); break; }
                        case 'b': { postfixStr=postfixStr.append("aCos"); break; }
                        case 'd': { postfixStr=postfixStr.append("aTan"); break; }
                        case 'e': { postfixStr=postfixStr.append("aLog"); break; }
                       }
                   }    
               }        
                ...
                if (tmpChar>='A' && tmpChar<='Z')
                   {
                    ...
                    if (j==count) { var[count]=tmpChar; count++; }
                   } 
               }
        ...
        public double firstDerivative(Hashtable values, char variable, double h) throws Exception
          {
           ...
           atValue1 = _value.doubleValue() - h; atValue2 = _value.doubleValue() + h; 
              
           values.remove(key); values.put(key, new Double(atValue1));
           double valueAt1 = evaluator(values);
           
           values.remove(key); values.put(key, new Double(atValue2));
        ...
        public double secondDerivative(Hashtable values, char variable, double h) throws Exception
          {
           ...
           atValue1 = _value.doubleValue() - h; atValue2 = _value.doubleValue(); atValue3 = _value.doubleValue() + h; 
           ...
           values.remove(key); values.put(key, new Double(atValue3));
