   ...
   public StylisPanel() throws Exception
          {
           ...
           shadowX = shadowY = 1; shadowColor = Color.darkGray;                      
           isInProcess=false;
           isAnimate=true;
           isStopInEnd=false;
          }
           ...
           if (textList!=null && textList.length!=0)
              {
               ...
               tl_textList.clear();
               for (int i=0; i<tl_count; i++) { tl_textList.addElement(textList[i]); }               
              }
        ...
        public void initListAnimation()
          {
           ...
           for (int i=0; i<tl_count; i++)
               {
                try { tl_textListX[i]=(String)tl_textList.get(i); }
        ...
        public void paint(Graphics graphics)
          {
           ...
           if (!isLineDraw) 
              {
               if (isStopInEnd) { isAnimate=false; }
               initListAnimation();
              } 
           else             refY-=1;
